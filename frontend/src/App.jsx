...
function FieldOfficerPortal({session,onLogout}) {
  ...
  return <div className="app-shell field-portal-shell">
    <aside className="sidebar field-sidebar">
      ...
      <nav className="side-nav" aria-label="Field officer navigation">{nav.map(([item,icon])=><button key={item} aria-label={item} title={item} className={view===item?'active':''} onClick={()=>setView(item)}><Icon name={icon} size={18}/><span>{item}</span>{item==='Alerts'&&issuedAlerts.length>0&&<b className="nav-count">{issuedAlerts.length}</b>}</button>)}</nav>
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="side-nav" aria-label="Main navigation">
        {NAV.map(item=><button key={item} aria-label={item} title={item} className={view===item?'active':''} onClick={()=>setView(item)}><Icon name={NAV_ICONS[item]} size={18}/><span>{item}</span>{item==='Reports & Alerts' && attention>0 && <b className="nav-count">{attention}</b>}</button>)}
      </nav>
      <div className="sidebar-spacer"/>
    ...
    <div className="app-frame">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button className="btn btn-ghost small portal-logout" aria-label="Logout" title="Logout" onClick={onLogout}><Icon name="logout" size={16}/><span>Logout</span></button>
        </div>
      </header>
...
function AlertsPane({alerts,auth,locations,onRefresh}) {
  ...
  async function transition(a,to){setError('');setNotice('');setBusy(`${a.id}-${to}`);try{await patch(`/api/alerts/${a.id}/transition`,{to_status:to,note:`Transitioned from PRAHARI command center`});await onRefresh();}catch(e){setError(e.message);}finally{setBusy(null);}}
  async function issueAndNotify(a){
    setError('');setNotice('');setBusy(`${a.id}-notify`);
    try{
      ...
      const summary=`${out.accepted_or_queued} accepted/queued for ${out.target_label}; ${out.failed} failed; ${skipped} already attempted and skipped.`;
      if(out.failed){
        const reasons=[...new Set((out.results||[]).filter(r=>r.error||r.error_code).map(r=>`${r.error_code?`Twilio ${r.error_code}: `:''}${r.error||'See delivery details.'}`))];
        setError(`${summary} ${reasons.join(' ')}`);
      }else setNotice(`${summary} Delivery confirmation updates separately.`);
      await loadDeliveries(a);
      await onRefresh();
